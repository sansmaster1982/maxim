...
Про диагностику начистоту: в текущей тестовой версии включён локальный диагностический лог. Приложение пишет ход работы (опкоды протокола, события чатов) в файл `Documents/maxim_diag.log` и открывает папку через «Файлы» (`UIFileSharingEnabled`) — это нужно, чтобы разбирать баги по логам на этапе беты. Лог остаётся на устройстве, в сеть не уходит, токен в нём замаскирован. В стабильном релизе диагностика будет отключена.

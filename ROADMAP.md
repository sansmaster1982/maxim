...
- [ ] **Этап 5 — Расширенные функции и устойчивость.** Детали: [docs/STAGE_05.md](docs/STAGE_05.md).
  - [x] DNS-over-HTTPS fallback (1.1.1.1/8.8.8.8 по IP, SNI и проверка сертификата сохранены) — против DNS-блокировки оператором.
  - [ ] Реакции (send), события групп/каналов (`_type=CONTROL`), поиск, доводка настроек, zstd-распаковка кадров (`cof=0xFF`).
  - [ ] Push-опкоды 130/142/155 и inbound typing — требуют живого pcap: эталонные клиенты (bridge.py, maxclient) их не парсят, угадывать поля нельзя.
- [ ] **Этап 6 — Заливка на GitHub.** Репозиторий готов (ветка `main`, чистое дерево, по коммиту на этап). Нужен `gh auth login` или `GH_TOKEN` — действие пользователя. Точные шаги: [docs/GITHUB.md](docs/GITHUB.md).

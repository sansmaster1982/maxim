...
### Проверка
Прямой запрос к серверу: LOGIN cof=4 (LZ4), 237КБ → 788КБ распаковано → распарсилось в `{profile, chats[4], messages, contacts, presence, config, token, updates}`. Чаты с именами и текстами читаются корректно (UTF-8): «А че», «Александр бот» и т.д.
...
## Этап 9 — фиксы по прогону на устройстве (2026-06-06)

Версия `0.1.1`. Найдено при тесте на телефоне, источник готовых решений —
сиблинг-проект `max new maxim` (Flutter, Android), сверен и переиспользован.

### 1. Контакты и заголовки чатов показывали сырой дамп `[{name:...}]`
Сервер MAX отдаёт имя не плоским `name`, а списком `names`
(`[{name,firstName,lastName,type}]`, type CUSTOM/ONEME). Старый код звал
`names.toString()`. Добавлен `lib/data/max/contact_name.dart` →
`displayContactName` (CUSTOM → ONEME → первый; внутри `name` либо
`firstName+lastName`). Применён в `max_client._parseContact`,
`ContactsRepository.refresh`, `snapshot` (чаты/контакты/peerName/профиль).
Тест `contact_name_test` (6 кейсов). Чинит сразу и контакты, и импорт
(импортированные показывались тем же артефактом).

### 2. Пароль 2FA нельзя было ввести буквами
На Samsung Keyboard `TextInputType.visiblePassword` открывает цифровой пад.
Заменено на `TextInputType.text` в `login_screen` (поле осталось obscure).

### 3. Анти-бан: реалистичный профиль устройства
На iOS вход по SMS идёт как ANDROID, но `androidInfo` бросает, и userAgent
отдавал `deviceName="Android"` + sdk34 + разрешение экрана iPhone — несуществующий
Android-телефон. Fallback заменён на согласованный пресет реального устройства
(Samsung SM-A546E, Android 14, arm64-v8a, 1080x2340). Реальные значения
по-прежнему берутся при наличии нативного Android-канала.

### Проверка
`flutter analyze` чисто, 72 теста зелёные. Три отдельных коммита (по фиксу).
Версия поднята `0.1.0 → 0.1.1` для нового билда TestFlight.

---

## Этап 10 — корень банов номера: нет keepalive (2026-06-06)

Версия `0.1.2`. Подробности: `docs/STAGE_10.md`.

### Причина
Сравнение клиентов: `test5.py` не банит, три наших (iso/new/exe) банят. Общее у
банящих и отсутствующее в test5 — авто-reconnect. `test5` и `max exe python`
шлют ОДИНАКОВЫЙ минимальный userAgent, но банит только второй → userAgent не при
чём; deviceId тоже (test5 со случайным id не банит). Корень: никто не слал
keepalive. Официальный клиент шлёт PING (opcode 1, декомпил `ewc.java`). Без
PING сервер рвёт idle-сокет, reconnect сбрасывает задержку в 2с → INIT+LOGIN
каждые ~30с → шторм сессий на номере → антифрод банит.

### Фикс (max iso)
PING раз в 25с пока сокет жив (`MaxOp.ping=1`, `Timer.periodic`, старт в connect
/ стоп в _disconnect). Reconnect-база 2с→5с + джиттер 0–1500мс. analyze чисто,
72 теста зелёные.

### Оговорка
Уже забаненный номер клиентом не лечится — фикс против БУДУЩИХ банов, проверять
на свежем номере. Те же правки нужны max new maxim и max exe python, если ими
пользоваться (в этом этапе не делались).

---

## Этап 11 — порт анти-бана из рабочего форка maxim-messenger (2026-06-06)

Версия `0.1.3`. Подробности: `docs/STAGE_11.md`. Источник истины — форк
`sansmaster1982/maxim-messenger` (не банит). Перенос хирургический (наши фичи
сохранены), сверен двумя воркфлоу (извлечение+план, затем review диффа) +
78 тестов.

### Что добавлено сверх Этапа 10
- **Правила 1+3** — `reconnect_policy.dart` (`ReconnectPolicy`): пол 1 LOGIN/30с
  (`_sinceLogin` — время с авторизации), backoff 5с/cap 5мин, предохранитель
  ≥6 попыток/5мин → 8мин, `rearm()` после LOGIN (чинит залипание авто-reconnect
  после мёртвого токена).
- **Правило 2** — keepalive op1 PING → **op16 PROFILE** (как в форке; ping-опкод
  не подтверждён), старт в `login()`, стоп в `_handleDrop`, `MaxOp.ping` удалён.
- **Правило 6** — `MaxRejected` + статус `rejected` + guard пустого payload +
  `cmd=3`-классификация + дроп перманентного отказа из outbox (не реконнект-петля).

### Поправка
Правило 5 (IOS/APNS) — ложная премиса: форк на iPhone логинится как ANDROID/GCM,
не IOS/APNS (проверено по коду). Не меняли. Правила 4 (deviceId) и 7 (сессии
op96/97) у нас уже выполнены, сохранены.

### Проверка
`flutter analyze` чисто, 78 тестов зелёные (+6 на `ReconnectPolicy`). Два
компилируемых коммита (основа rule 6, затем правила 1/2/3/6).

---

## Этап 12 — переезд на master форка + марафон фиксов (0.2.0–0.2.8, 2026-06-07/08)

Подробно: `docs/STAGE_12.md`. Ветка `adopt-master`. Взяли master форка
maxim-messenger за основу (lib/test/bin), iOS-обвязку оставили свою. Настроили
device-логи без Mac (`pymobiledevice3` + файл-лог, т.к. iOS release `print` не
идёт в syslog; `UIFileSharingEnabled` для afc-pull лога и SQLite-базы). По
device-логу добили цепочку багов:
- 0.2.2/0.2.3 — чистка БД при смене аккаунта (чужие чаты с прежней SIM).
- 0.2.4 — гонка login/подписки на synced-chats (кеш+дотягивание).
- **0.2.5 — НЕСОВМЕСТИМОСТЬ СХЕМЫ БД (v7-vs-v7, разные колонки)** → дроп+recreate
  на v8. Из-за неё молча не слались сообщения (insert падал на `no such column
  cid` вне try) и не строились имена. Подтверждено дампом базы.
- 0.2.6 — имя из `myProfileId` (своё имя протекало во все чаты).
- 0.2.7 — дедуп дублей диалога (контакт-строка + синк-строка).
- 0.2.8 — фича «Изменить имя» (op16 PROFILE, payload из декомпила r2e.java).

Анти-бан проверен на устройстве: 1 LOGIN/сессия, keepalive op16/25с, без штормов;
reconnect_policy не ослаблялся. Предстоит: переименование собеседника (op34),
ошибка добавления чата, скролл чата к последнему сообщению, мерж в `main`,

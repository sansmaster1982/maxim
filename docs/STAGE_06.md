...
`MaxClient` принимает `zstdDecoder` — подключаемый декодер кадров cof=0xFF. По реверсу `one.me.sdk.zsrd.ZstdUtil.nativeDecompress(byte[])` это стандартный zstd-кадр БЕЗ словаря (одноаргументный JNI, без `ZDICT`/`usingDict`), поэтому подойдёт любой generic zstd. Передаётся в конструктор — клиент не навязывает нативную зависимость iOS-сборке. По умолчанию null: такие (редкие) кадры логируются и отдаются сырыми, основной трафик идёт LZ4. Остаётся лишь опциональная упаковка платформенного zstd (FFI/плагин) — это шаг сборки, не кода.
